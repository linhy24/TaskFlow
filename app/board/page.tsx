...
const TaskPage: React.FC = () => {
  const [boards, setBoards] = useState<Board[]>([
    ...
    const handleDragEnd = useCallback((event: any) => {
      ...
      const activeBoardIndex = boards.findIndex(board => board.tasks.some(task => task.id === active.id));
      const overBoardIndex = boards.findIndex(board => board.id === over.id.split('-')[0]);
      const activeTaskIndex = boards[activeBoardIndex].tasks.findIndex(task => task.id === active.id);
      const overTaskIndex = boards[overBoardIndex].tasks.findIndex(task => task.id === over.id);

      if (overBoardIndex !== activeBoardIndex || activeTaskIndex !== overTaskIndex) {
          setBoards(prevBoards => {
              const newBoards = [...prevBoards];
              const [movedTask] = newBoards[activeBoardIndex].tasks.splice(activeTaskIndex, 1);

              if (over.id.startsWith('board-')) {
                  newBoards[overBoardIndex].tasks.push(movedTask);
              } else {
                  newBoards[overBoardIndex].tasks.splice(overTaskIndex, 0, movedTask);
              }
  ...
  const handleAddTask = () => {
    if (inputValue) {
        setBoards(prevBoards => {
            const newBoards = [...prevBoards];
            const newTask = { id: `task-${Date.now()}`, content: inputValue, isNew: true }; // Using Date.now() for unique ID
            newBoards[0].tasks.push(newTask); // Add to the first board by default
            return newBoards;
        });
        setInputValue('');
    }
                ...
                <div className="w-full max-w-5xl p-4 ">
                    <div className="flex justify-center items-center mb-6 space-x-4 ">
                        ...
                        {boards.map((board, index) => (
                            <SortableContext key={board.id} items={board.tasks.map(task => task.id)} strategy={verticalListSortingStrategy}>
                                <div className="p-4 rounded-lg dark:bg-zinc-800/30 dark:border-neutral-800 backdrop-filter backdrop-blur-lg shadow-lg">
                                    <h2 className="font-mono text-lg font-semibold mb-2">Board {index + 1}</h2>
